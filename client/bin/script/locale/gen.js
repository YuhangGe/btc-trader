...
async function readLocale(locale, mDir, prefix = '') {
  ...
  for(let k = 0; k < files.length; k++) {
    ...
    try {
      ...
      for(const key in dict) {
        ...
        if (output.hasOwnProperty(okey)) {
          throw new Error(`Locale dict key ${okey} duplicated in file: ${file}`);
        } else {
          loop(file, output, okey, dict[key]);
        }
      }
    } catch(ex) {
  ...
  function loop(file, output, k, v) {
    if (typeof v === 'object' && v !== null) {
      for(const sk in v) {
        loop(file, output, `${k}.${sk}`, v[sk]);
      }
    } else if (output.hasOwnProperty(k)) {
      throw new Error(`Locale dict key ${k} dumplicated in file: ${file}`);
    } else {
      output[k] = v;
    }
  }
...
module.exports = async function generate(changedFile = null) {
  ...
  // }
  if (!changedFile || changedFile.startsWith(modulePath)) {
    const arr = await Promise.all(
      (await _util.readdir(modulePath))
        .map(cDir => readModule(cDir, modulePath))
    );
